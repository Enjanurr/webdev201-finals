...
export default function EditProfile() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (profilePhoto) formData.append('profilePicture', profilePhoto);  // Append profile photo if available
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/update/editProfile`, {
        method: 'PATCH',
        body: formData,
        credentials: 'include',  // Optional, if you need session cookies for authentication
      });
      ...
      setTimeout(() => {
        router.push('/borrowedBooks'); // Redirect to profile or dashboard after update
      }, 1500);
    } catch (err) {
  ...
  return (
    ...
    >
      <h2 className="text-3xl font-bold mb-6 text-center text-zinc-800 dark:text-white">Edit Profile</h2>
