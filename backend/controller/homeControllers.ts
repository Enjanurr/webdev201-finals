...
export const borrow = async (
  ...
  try {
    ...
    if (!book) {
      return res
        .status(400)
        .json({ message: "Book not available or already borrowed" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Book borrowed successfully", book: updatedBook });
  } catch (error) {
...
export const borrowedBooksAndProfile = async (
  req: CustomRequest,
  res: Response
): Promise<any> => {
  try {
...
export const returned = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Returned successfully", returnedBook: returned });
  } catch (error) {
    console.error("Failed to return book", error);
    return res
      .status(500)
      .json({ message: "Failed to return the book", error });
  }
};

export const dummyreturn = async (
  req: Request,
  res: Response
): Promise<any> => {
  try {
